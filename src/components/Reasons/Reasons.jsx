        ...
        <div className='flex gap-5'>
            {
                reasons.map((reason, index)=>{
                    return(
                        <div key={index} className='w-[300px] h-80 rounded-2xl p-10 bg-linear-to-b from-blue-950 to-gray-900 relative '>
                            <div className=' font-bold text-[22px] '>{reason.title}</div>
                            <div className=' py-5 text-gray-400 text-[16px] '>{reason.detail}</div>
                            <div className=' text-7xl right-0 mr-10 absolute opacity-60 bottom-0 mb-5 '>{reason.icon}</div>
                        </div>
                    )
                })
            }
